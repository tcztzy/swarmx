...
| `packages/core/src/swarm.ts` | Internal workflow graph: materializes config, derives nested Agent topology, scopes ablation-controlled services to direct Agents, detects cycles, evaluates edges, waits for predecessors, enforces the step bound, and collects ordered chunks/metrics. Construction is owned by `core-runtime.ts`. `net`/`proc` via nodes |
