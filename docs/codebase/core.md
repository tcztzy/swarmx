...
| `packages/core/src/autonomy.ts` | Engineering lifecycle and analysis-specific state machines layered above the generic task runtime, plus transition guards, circuit breakers, validators, evidence, and domain path helpers. `pure` + `fs` path constants |
