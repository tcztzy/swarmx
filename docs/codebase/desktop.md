...
| `packages/desktop/src/main/ipc.ts` | Main IPC router/authorization boundary; maps transport evidence to `ipc.request` plus normalized audit policy, records semantic effects and content-free finalized Context manifests (profile/fidelity/projection/evidence/summary-call/budget/checkpoint metadata), validates sender, durably starts and settles digest-bound Session requests before Provider/tool/ACP execution, replays settled request receipts and rejects unknown/conflicting/active duplicates, closes foreground output and pending interactions before request-correlated Session persistence, binds required Extension services to persisted Session Project identity/root, de-duplicates bounded bootstrap receipts across Provider-key attempts, and disables all Agent-facing Extension MCPs in read-only side chats; injects global Memory snapshots and Session-isolated reflection reminders, exposes local Memory/Reference tools only to SwarmX-owned execution, brokers explicit Agent memory mutation confirmation, exposes list/cancel/decision-only WorkItem controls, and excludes raw arguments/results from audit. `ipc` + `fs` + all privileged effects |
| `packages/desktop/src/main/ipc-router.ts` | Contract-aware registrar layered over the authorized/audited Main handler: validates argument tuples before dispatch, validates sync/async results before return, rejects policy mismatches without exposing payloads in errors, and passes an explicit per-dispatch receipt for semantic-only audit de-duplication. `ipc` |
...
| `packages/desktop/src/main/agent-chunk-publisher.ts` | Batches/throttles bounded live agent/terminal chunks, sends only while the owner window exists, and permanently rejects publication after the foreground completion barrier closes it. `ipc` |
...
| `packages/desktop/src/main/app-update-ipc.ts` | App Update feature router: registers two audited invoke contracts and validates update events before publishing. It receives a registrar and update service, never raw Electron registration authority. `ipc` |
| `packages/desktop/src/main/browser-ipc.ts` | Embedded Browser feature router: registers eight invoke contracts, preserves shared Browser/Terminal owner cleanup, and projects Host state to bounded Renderer DTOs. It receives the audited registrar and owns no Electron registration authority. `ipc` |
| `packages/desktop/src/main/global-memory-ipc.ts` | Global Memory feature router: binds the compatibility-named `personalMemory:*` read/save/forget channels to the current `USER.md` / `MEMORY.md` service through strict browser-safe contracts and the audited registrar. `ipc` |
| `packages/desktop/src/main/global-memory-service.ts` | Authoritative Global Memory application service: overlays legacy migration input on reads, carries observed revisions into backend mutations, clears the overlay after authoritative `USER.md` changes, freezes execution snapshots, and retains bounded per-Session reflection cursors. `fs` through Memory and settings |
| `packages/desktop/src/main/project-ipc.ts` | Project feature router: binds the eight Project invoke contracts to a Project service through the audited registrar and owns no Electron registration authority. `ipc` |
| `packages/desktop/src/main/project-service.ts` | Main Project orchestration: native folder selection/creation, canonical Core registry operations, reveal, and running-Session exclusion plus side-chat cleanup for archival. `fs` + `ipc` |
| `packages/desktop/src/main/terminal-ipc.ts` | Interactive Terminal feature router: registers four semantic-only invoke contracts, preserves composition-root owner cleanup, and injects strict Main-side data/exit event projection into the PTY Host. It receives the audited registrar and owns no Electron registration authority. `ipc` + `proc` |
| `packages/desktop/src/main/workspace-inspection-ipc.ts` | Workspace inspection feature router: registers root/review/list/read contracts through the audited registrar, resolves the requested working directory, and projects host-only WorkspaceTools results to the narrower Renderer DTOs. Native file/folder selection remains outside this read-only slice. `fs` + `ipc` |
| `packages/desktop/src/main/workspace-tool-permissions.ts` | Direct Workspace-tool permission adapter: resolves layered policy, classifies read/write/execute access, wraps function/text calls, routes Auto review to fail-closed human fallback, and emits only bounded redacted approval summaries. `pure` |
...
| `packages/desktop/src/main/browser-host.ts` | Embedded browser view lifecycle, bounds/navigation normalization, permission handling, owner-scoped state, and an injectable state projection applied before Renderer publication. `net` + `ipc` |
...
| `packages/desktop/src/main/lsp-host.ts` | Resolves the owning Extension and reruns installed-state composition preflight before spawning JSON-RPC language servers, serves bounded file/skill completions, uses a Provider-secret-free environment whitelist, and handles cancellation/shutdown. `proc` + `fs` |
...
| `packages/desktop/src/main/personal-memory.ts` | Legacy settings-backed Personal Memory service and confirmation-gated Agent tool retained only as a compatibility/migration surface. It does not own authoritative Global Memory. `fs` through settings |
| `packages/desktop/src/main/private-json-file.ts` | Main-only mechanical writer for private JSON documents: exact pretty JSON, unique exclusive temporary files, restrictive modes, atomic replacement, file/directory fsync, and owned-temp cleanup. Domain schemas, queues, and conflict policy stay in each store. `fs` |
...
| `packages/desktop/src/main/memory-runtime-backend.ts` | Implements the async Core entity and global Memory backends by mapping strict file/CRUD/search/version requests to private Memory MCP calls and rebuilding graph projection from the bounded snapshot. `proc` through host |
| `packages/desktop/src/main/memory-runtime-service.ts` | Lazy app-lifecycle owner for verified Memory runtime launch. It exposes entity plus global-file Core backends and owns no fallback format or migration authority. `fs` + `proc` |
...
| `packages/desktop/src/main/task-runtime-ipc.ts` | Task Runtime feature router: exposes only list/cancel/decision contracts, constructs the corresponding Supervisor commands in Main, validates operation-specific receipts, and owns no launch or token authority. `ipc` |
...
| `packages/desktop/src/main/provider-auth.ts` | Schema-version-2 user-editable Provider auth file store with queued in-instance mutations, restrictive permissions, and credential lookup; plaintext never leaves Main. `fs` + `secret` |
...
| `packages/desktop/src/main/session-messages.ts` | Converts Core Session events into desktop message streams, publishes chunks, handles timing/interruption, asserts final assistant output, and applies Core's shared exclusion of persisted Memory and Project-bootstrap receipts from subsequent model history. `ipc` |
...
| `packages/desktop/src/main/terminal-host.ts` | Owner-scoped PTY lifecycle, injectable data/exit event projection, and fail-closed semantic audit actions `create`/`write`/`resize`/`close`/`exit`; close reasons distinguish user kill, owner cleanup, and app disposal. Records byte counts/lifecycle only, never terminal data/cwd/env. `proc` + `ipc` |
...
| `packages/desktop/src/main/workspace-shell.ts` | Bounded shell/PTY sessions: explicit OS-sandbox strategy, sanitized environment, Project-root execution, output limits, polling, cancellation, process-group termination, and agent-tool adapter. `proc` + `fs` |
| `packages/desktop/src/main/workspace-tools.ts` | Direct Project tools for read/list/search/write/edit/delete/patch, review, browser, LSP, child agents, monitors, and cron; derives model-facing JSON Schema and call-time validation from shared Zod definitions, then enforces containment, digests, limits, and atomic writes while delegating permission wrapping to the dedicated adapter. `fs` + `proc` |

## Preload and shared transport

| Source | Contract |
| --- | --- |
| `packages/desktop/src/preload/api.ts` | Typed IPC invoke/subscribe functions, bootstrap parser, global Memory controls, list/cancel/decision WorkItem controls, verified audit calls, and the finite `SwarmxDesktopApi` surface. `ipc` |
...
| `packages/desktop/src/shared/desktop-api.ts` | Renderer-safe DTOs/events for Sessions, chunks, global Memory, WorkItem observation/control, media, browser, workspace, permissions, Providers, extensions, updates, audit, and all API methods. `pure` |
| `packages/desktop/src/shared/ipc-contracts/base.ts` | Browser-safe invoke/event contract primitives, explicit audit-policy type, and duplicate-rejecting feature-map composition. `pure` |
| `packages/desktop/src/shared/ipc-contracts/app-update.ts` | Canonical App Update state schema/type plus its two invoke and one event contracts. `pure` |
| `packages/desktop/src/shared/ipc-contracts/browser.ts` | Canonical bounded Browser create/bounds/state DTOs, all eight invoke contracts, the owner-scoped state event contract, and the Renderer Browser API surface. `pure` |
| `packages/desktop/src/shared/ipc-contracts/global-memory.ts` | Canonical strict Global Memory state/input schemas, optional observed-revision compatibility fields for conflict-safe current clients, compatibility-named read/save/forget invoke contracts, and the Renderer Global Memory API surface. `pure` |
| `packages/desktop/src/shared/ipc-contracts/project.ts` | Canonical strict Desktop Project DTO plus all eight Project invoke argument/result/audit contracts, derived from the browser-safe Core Project record schema. `pure` |
| `packages/desktop/src/shared/ipc-contracts/terminal.ts` | Canonical Terminal create/write/resize/kill DTOs, all four semantic-only invoke contracts, two owner-scoped event contracts, and the Renderer Terminal API surface; Host-owned normalization and rejection remain outside transport. `pure` |
| `packages/desktop/src/shared/ipc-contracts/task-runtime.ts` | Canonical browser-safe WorkItem list/cancel/decision DTOs and invoke contracts, derived from Core Task Runtime and worker-payload schemas without Supervisor process authority. `pure` |
| `packages/desktop/src/shared/ipc-contracts/workspace-inspection.ts` | Canonical strict Workspace review/list/preview DTOs plus the four read-only inspection invoke contracts; excludes host-only read digests and native picker authority. `pure` |
| `packages/desktop/src/shared/ipc-contracts/index.ts` | Feature-composed Desktop invoke/event registries; migrated channels have exactly one runtime contract. `pure` |
...
| `packages/desktop/src/renderer/src/App.tsx` | Top-level application composition: navigation, padded conversation surface with a floating composer overlay, global Memory settings, workflow, extensions, runtime, profile, terminal, and GUI contribution registry; delegates Doctor transient state and effects to the shared feature controller while retaining panel/layout composition. `ui` |
...
| `packages/desktop/src/renderer/src/conversation-history.tsx` | Renders normalized message/render events, always-visible global/legacy Memory use receipts, streaming progress, tool calls, edits, forks, attachments, and copy controls. `ui` |
...
| `packages/desktop/src/renderer/src/doctor-controller.ts` | Pure `/doctor` and `/setup` command parsing plus the single injected-API React controller shared by the right-side Doctor panel and Runtime Settings; owns generation-safe transient inspection/version/install/fix state, preserves scoped/global cache separation, and performs no unconfirmed repair. `ui` + `ipc` |
| `packages/desktop/src/renderer/src/doctor-panel.tsx` | Presentation-only right panel for Harness requirements, versions, Doctor issues, and setup/fix state supplied by the shared controller. `ui` |
...
| `packages/desktop/src/renderer/src/internal-terminal.tsx` | Bottom terminal panel markup and theme, backed by the shared typed Terminal lifecycle controller. `ui` + `ipc` |
| `packages/desktop/src/renderer/src/terminal-controller.ts` | Shared xterm lifecycle hook for both Terminal surfaces: generation-safe create/restart, buffered input, owner event filtering, resize deduplication, and exact PTY cleanup. `ui` + `ipc` |
...
| `packages/desktop/src/renderer/src/runtime-settings.tsx` | Runtime environment/Doctor presentation supplied by the shared Doctor controller plus detached WorkItem status, cancel, and pending-decision UI; the owning App still refreshes the read-only task projection while this surface is visible. `ui` + `ipc` |
...
| `packages/desktop/src/renderer/src/workspace-panel.tsx` | Files/review/terminal/browser panel, diff parsing, file previews, and workspace navigation; its Terminal view delegates lifecycle control to the shared hook. `ui` + `ipc` |
