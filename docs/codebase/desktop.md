...
| `packages/desktop/src/main/reference-library-host.ts` | Main-only private MCP host for explicitly configured `swarmx-ref` ZIM, SearXNG Web, and local Zotero sources; validates launch inputs, uses a credential-free environment, verifies the exact server/tool surface, enforces structured/text consistency and bounds, and closes with the app. `proc` + `fs` + `net` through module |
