...
| `packages/desktop/src/main/ipc.ts` | Main IPC router/authorization boundary; maps transport evidence to `ipc.request` plus normalized audit policy, records semantic effects and content-free finalized Context manifests (profile/fidelity/projection/evidence/summary-call/budget/checkpoint metadata), validates sender, injects global Memory snapshots and Session-isolated reflection reminders, exposes local Memory/Reference tools only to SwarmX-owned execution, brokers explicit Agent memory mutation confirmation, exposes list/cancel/decision-only WorkItem controls, and excludes raw arguments/results from audit. `ipc` + `fs` + all privileged effects |
