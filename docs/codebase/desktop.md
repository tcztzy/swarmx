...
| `packages/desktop/src/main/ipc.ts` | Main IPC router/authorization boundary and owner of the shared Desktop Core Cordis Runtime. It injects the runtime `harnessCatalog` into extension inventory, composition plans, Doctor environment, and Session-discovery checks. Workflow, Agent Composition, and ACP Session discovery execute through that root's request Fibers; Main shutdown disposes it. Existing audit, Project authority, Memory/Reference tool, and permission boundaries remain enforced. `ipc` + `fs` + all privileged effects |
...
| `packages/desktop/src/renderer/src/App.tsx` | Top-level application composition: navigation, padded conversation surface with a floating composer overlay, global Memory settings, workflow, extensions, runtime, profile, terminal, and GUI contribution registry; recognizes the managed Codex module as a protected Harness while retaining legacy custom-adapter recognition; delegates Doctor transient state and effects to the shared feature controller while retaining panel/layout composition. `ui` |
...
| `packages/desktop/src/renderer/src/workflow-workspace.tsx` | Workflow JSON parsing, managed and legacy ACP Harness identification, node/edge graph UI, import validation, and execution controls for canonical `SwarmConfig`. `ui` + `ipc` |
