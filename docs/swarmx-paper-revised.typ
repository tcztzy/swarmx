...
// 中文：生成文件首先作为对话中的可见结果出现。Science 工具返回的工件定位符在同一完成回合中被恢复、去重和排序，并形成可点击卡片。侧栏预览只请求宿主授权、摘要校验且大小受限的文本、图像、表格或 PDF 表示。浏览器接收相对标识和有界内容，不接收绝对主机路径。普通 Markdown 相对链接仍是最终回答中引用交付文件的首选形式，从而使用户在离开 SwarmX 时仍能理解文件关系。
...
content, not absolute Host paths. Ordinary relative Markdown links remain the preferred form for
referencing deliverables in a final answer, so file relationships remain understandable outside
SwarmX.
