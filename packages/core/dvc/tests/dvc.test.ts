...
async function mounted(config: Record<string, unknown> = {}) {
  ...
  const fiber = await context.plugin(DvcService, config as never);
...
describe("dsh-dvc", () => {
  it("returns bounded, path-free local status and manifest digests", async () => {
  ...
  it("pulls only on an explicit validated request", async () => {
    const root = repository();
    const { context, fiber } = await mounted({ command: fakeDvc() });
    try {
      expect(existsSync(join(root, ".fake-dvc-pulls"))).toBe(false);
      const result = await context.dvc.pull(root, { targets: ["data/raw.dvc"], remote: "origin" });
  ...
  it("reproduces exact clean HEAD in a retained disposable worktree", async () => {
    const root = repository();
  ...
  it("rejects dirty sources and returns redacted failed stage outcomes", async () => {
    const root = repository();
    const { context, fiber } = await mounted({ command: fakeDvc() });
    try {
  ...
  it("cleans retained reproductions on disposal and keeps missing DVC lazy", async () => {
    const root = repository();
