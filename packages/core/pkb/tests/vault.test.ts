...
import { PkbError, PkbVault } from "../src/index.js";

const roots: string[] = [];

async function temporaryRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "swarmx-pkb-test-"));
  roots.push(root);
  return root;
}
...
async function fixture() {
  ...
  const vault = new PkbVault({ root: vaultRoot });
...
describe("PkbVault", () => {
  ...
  it("V130 V131 V136 V137: creates, revises, indexes, logs, and deprecates a concept", async () => {
    ...
    const created = await vault.createConcept(workspace, {
      body: "# 决定\n\nPKB 使用 Markdown。",
      description: "PKB 使用开放 Markdown 作为持久知识。",
      scope: "workspace",
      tags: ["pkb", "架构"],
      title: "PKB 使用 Markdown",
      type: "Decision",
    });
    ...
    const updated = await vault.updateConcept(workspace, {
      body: "# 决定\n\nPKB 使用 OKF v0.2 Markdown。",
      description: "PKB 使用 OKF v0.2 Markdown 作为持久知识。",
  ...
  it("V132: rejects foreign workspace ids and symlinked concept files", async () => {
    ...
    await expect(vault.readConcept(otherWorkspace, concept.id)).rejects.toBeInstanceOf(PkbError);
