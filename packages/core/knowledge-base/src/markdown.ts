...
export type KnowledgeBaseConceptMetadata = z.infer<typeof metadataSchema> & {
...
export interface ParsedConcept {
  ...
  readonly metadata: KnowledgeBaseConceptMetadata;
  readonly revision: string;
}

function invalid(message: string, cause?: unknown): KnowledgeBaseError {
...
function validatePortableBody(body: string, sources: readonly KnowledgeBaseSource[]): void {
  if (/\[\[[^\]]+\]\]/u.test(body)) {
    throw invalid("knowledge base concepts must use standard Markdown links, not Wikilinks");
  }
  if (/^\s*\^[a-z0-9-]+\s*$/imu.test(body)) {
    throw invalid("knowledge base concepts must not use Obsidian block references");
  }
  if (/<\/?(?:script|iframe|object|embed)\b/iu.test(body)) {
    throw invalid("knowledge base concepts must not contain executable embedded HTML");
  }

  const ids = new Set<string>();
  for (const source of sources) {
    if (source.id !== undefined) {
      if (ids.has(source.id)) throw invalid(`Duplicate knowledge base source id '${source.id}'`);
      ids.add(source.id);
    }
  }
}

function normalizeMetadata(input: unknown): KnowledgeBaseConceptMetadata {
  let parsed: z.infer<typeof metadataSchema>;
  try {
    parsed = metadataSchema.parse(input);
  } catch (error) {
    throw invalid("Invalid knowledge base concept frontmatter", error);
  }

  if (parsed.swarmx_scope === "workspace" && parsed.swarmx_workspace === undefined) {
    throw invalid("Workspace knowledge base concept requires swarmx_workspace");
  }
  if (parsed.swarmx_scope === "global" && parsed.swarmx_workspace !== undefined) {
    throw invalid("Global knowledge base concept must not carry swarmx_workspace");
  }
...
export function parseConcept(
  ...
  if (Buffer.byteLength(source, "utf8") > maxBytes) {
    throw invalid(`knowledge base concept exceeds ${String(maxBytes)} bytes`);
  }
  const match = /^---\n([\s\S]*?)\n---\n([\s\S]*)$/u.exec(source);
  if (match?.[1] === undefined || match[2] === undefined) {
    throw invalid("knowledge base concept requires one leading YAML frontmatter block");
  }

  const document = parseDocument(match[1], { uniqueKeys: true });
  if (document.errors.length > 0) {
    throw invalid("Invalid knowledge base concept YAML", document.errors[0]);
  }
  let value: unknown;
  try {
    value = document.toJS({ maxAliasCount: 0 });
  } catch (error) {
    throw invalid("Invalid knowledge base concept YAML value", error);
  }
...
export function renderConcept(metadata: KnowledgeBaseConceptMetadata, body: string): string {
