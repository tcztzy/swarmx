...
The same SVG markup is embedded in `src/harness-icon-data.ts` so reusable renderer consumers do not need to copy this public directory. SwarmX uses its own application glyph. OpenClaw intentionally uses the deterministic fallback because a revisioned, redistributable ACP/product icon was not verified; the official `openclaw acp` command itself is verified separately. The OpenClaw project currently declares MIT, while the registry-hosted assets above are Apache-2.0. All names and marks remain the property of their respective owners. See `packages/desktop/THIRD_PARTY_NOTICES.md` for complete notices and licenses.
