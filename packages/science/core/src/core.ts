...
import { ScienceResourceResolver } from "./resource-resolver.js";
import {
  MAX_RESOURCE_SELECT_SOURCE_BYTES,
  type ScienceResourceBatchHead,
  type ScienceResourceBatchHeadRequest,
  type ScienceResourceGetRequest,
  type ScienceResourceGetResult,
  type ScienceResourceHead,
  type ScienceResourceHeadRequest,
  type ScienceResourceNeighborsRequest,
  type ScienceResourceNeighborsResult,
  type ScienceResourceSelectRequest,
  type ScienceResourceSelectResult,
...
import type { ScienceProcessRuntime } from "./subprocess.js";
...
export interface ScienceWorkspace {
  readonly key: string;
  readonly root: string;
}

export type ScienceWorkspaceResolver = (sessionId: string) => ScienceWorkspace;
...
export const DEFAULT_MAX_CELL_OUTPUT_BYTES = 256 * 1024;
export const DEFAULT_MAX_EXPORT_BYTES = 5 * 1024 * 1024;
...
export const DEFAULT_TYPST_COMMAND = "typst";
export const DEFAULT_WRITING_PREVIEW_RUNTIME_COMMAND = resolve(
...
export const DEFAULT_TYPST_INITIAL_COMPILE_TIMEOUT_MS = 15_000;
export const DEFAULT_TYPST_MAX_DIAGNOSTICS_BYTES = 64 * 1024;
export const DEFAULT_TYPST_MAX_PDF_BYTES = 32 * 1024 * 1024;
export const DEFAULT_TYPST_MAX_SOURCE_BYTES = 1024 * 1024;
...
/** Platform-neutral workspace-scoped science authority. */
export class ScienceCore {
  ...
  private readonly resolveWorkspace: ScienceWorkspaceResolver;

  constructor(runtime: ScienceRuntime, config: Config, resolveWorkspace: ScienceWorkspaceResolver) {
    ...
    this.notebookRuntime =
      this.notebookRuntimeKind === "jupymcp"
        ...
        : new PythonRuntime(runtime.subprocess, {
  ...
  startRun(sessionId: string, request: StartRunRequest, signal?: AbortSignal): RunMutation {
    abortIfRequested(signal);
  ...
  finishRun(sessionId: string, request: FinishRunRequest, signal?: AbortSignal): ScienceRun {
    abortIfRequested(signal);
  ...
  compareRuns(sessionId: string, request: CompareRunsRequest, signal?: AbortSignal): RunComparison {
    abortIfRequested(signal);
  ...
  getWorkspace(sessionId: string, signal?: AbortSignal): ScienceWorkspaceSnapshot {
    abortIfRequested(signal);
  ...
  headResource(
    ...
    abortIfRequested(signal);
    const workspace = this.workspace(sessionId);
    const resolver = new ScienceResourceResolver(this.journal.getWorkspace(workspace.key));
    const parsed = parseRequest(scienceResourceHeadRequestSchema, request);
    return scienceResourceHead(resolver, parsed);
  }
  ...
  batchHeadResources(
    ...
    const workspace = this.workspace(sessionId);
    const resolver = new ScienceResourceResolver(this.journal.getWorkspace(workspace.key));
    const parsed = parseRequest(scienceResourceBatchHeadRequestSchema, request);
    return scienceResourceBatchHead(resolver, parsed);
  }
  ...
  getResource(
    ...
    const workspace = this.workspace(sessionId);
    const resolver = new ScienceResourceResolver(this.journal.getWorkspace(workspace.key));
    const parsed = parseRequest(scienceResourceGetRequestSchema, request);
    return scienceResourceMetadata(resolver, parsed);
  }
  ...
  selectResource(
    ...
    const workspace = this.workspace(sessionId);
    const resolver = new ScienceResourceResolver(this.journal.getWorkspace(workspace.key));
    const parsed = parseRequest(scienceResourceSelectRequestSchema, request);
    const resource = resolver.resolve(parsed.id);
    if (resource.kind !== "artifact") {
      throw new ScienceError(
        "Resource selection requires an artifact ID",
        "RESOURCE_KIND_MISMATCH",
      );
    }
    const preview = this.previewArtifact(sessionId, { artifactId: resource.entityId }, signal);
    return scienceResourceSelect(resolver, parsed, preview);
  }
  ...
  getResourceNeighbors(
    ...
    const workspace = this.workspace(sessionId);
    const resolver = new ScienceResourceResolver(this.journal.getWorkspace(workspace.key));
    const parsed = parseRequest(scienceResourceNeighborsRequestSchema, request);
    return scienceResourceNeighbors(resolver, parsed);
  }
  ...
  previewArtifact(
    ...
    if (artifact.mime.startsWith("text/") || artifact.mime === "application/json") {
      if (artifact.size > MAX_RESOURCE_SELECT_SOURCE_BYTES) {
        return { kind: "unavailable", ...identity, reason: "too-large" };
      }
      const text = this.artifacts.readText(
        artifact.digest,
        MAX_RESOURCE_SELECT_SOURCE_BYTES,
        signal,
      );
  ...
  private workspace(sessionId: string): { key: string; root: string } {
    return this.resolveWorkspace(String(sessionId));
  }
}
